...
need_mb() { case "$1" in
  *vitl*|*large*|*sam3*|*maxvit_b*|*swin_b*|*convnext_b*|*regnety120*|*_512*) echo 17000;;
  *) echo 10000;; esac; }
...
run_gpu() { local need=$1 log=$2; shift 2
  ...
  # 9>&- : the child must NOT inherit the lock fd. Without this the flock stays held
  # for as long as the training job runs (the fd is still open in the child), so the
  # release after LAUNCH_HOLD never took effect and later workers idled behind a
  # launch that had long finished -- GPUs sat half empty with recipes queued.
  CUDA_VISIBLE_DEVICES=$GPU "$@" 9>&- > "$log" 2>&1 & local pid=$!
